...
```bash
git clone https://github.com/0nyx-networks/ComfyUI-running-on-gpupods.git comfyui-running-on-gpupods
...
- `TAILSCALE_AUTHKEY`: Tailscale認証キー（設定するとTailscale VPN経由アクセスが有効になります。未設定の場合は無効）
- `TAILSCALE_HOSTNAME`: TailscaleのホストNAME（デフォルト: `comfyui-gpupod`）
- `TAILSCALE_TAG`: Tailscaleのタグ（デフォルト: `cloud-gpu-pods`）
...
### Tailscale経由でのアクセス（オプション）

`TAILSCALE_AUTHKEY` を設定することで、Tailscale VPNを経由したセキュアなアクセスが可能になります。  
[Tailscaleの管理コンソール](https://login.tailscale.com/admin/settings/keys)からAuthKeyを発行してください。

Tailscale有効時の動作：
- ComfyUIは `127.0.0.1` でListenし、`tailscale serve` 経由で各ポートを公開
- Tailscaleネットワーク上の他のデバイスからホスト名でアクセス可能
- ポートの直接公開（`-p 8188:8188`）は不要
...
podman container run -d --replace \
  ...
  --env TAILSCALE_AUTHKEY=tskey-auth-xxxxxxxxxxxxxxxx \
  --env TAILSCALE_HOSTNAME=comfyui-gpupod \
  --env TAILSCALE_TAG=cloud-gpu-pods \
  docker.io/m10i1986/comfyui-running-on-gpupods:latest
```

Tailscale接続後、以下のURLでアクセスできます：
```
http://comfyui-gpupod:8188
```
...
- `TAILSCALE_AUTHKEY`: Tailscale認証キー（設定するとTailscaleが有効になります）
- `TAILSCALE_HOSTNAME`: Tailscaleホスト名（デフォルト: `comfyui-gpupod`）
- `TAILSCALE_TAG`: Tailscaleのタグ（デフォルト: `cloud-gpu-pods`）
